...
```text
.
├── index.html      # 首页：记录、工作室档案、硬件清单、书架与研究路线
├── style.css       # 首页样式
├── main.js         # 首页导航、记录筛选、硬件筛选和书架检索
...
首页中的工作室档案、硬件清单和书架目录基于本地 `D:\Obsidian\工作室` 整理。原始目录只读，不属于本仓库的运行时依赖；硬件的完整状态以飞书使用清单为准，项目总览中的空白内容会在形成真实记录后继续补充。
