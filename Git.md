...
2. SSH方式管理多个账号（推荐）
    - 为每个账号生成不同的 SSH key：
       ...
       # 可指定不同文件名，如 id_ed25519_github1、id_ed25519_github2
       ```
    - 将公钥添加到对应 GitHub 账号。
    ...
    - 克隆或设置远程仓库时使用对应 Host：
       ```bash
       git clone git@github-账号1:username/repo.git
       git remote set-url origin git@github-账号2:username/repo.git
       ```

3. HTTPS方式切换（每次 push/pull 输入账号密码或使用 token，推荐 SSH）

更多细节可参考 GitHub 官方文档：
https://docs.github.com/zh/authentication/connecting-to-github-with-ssh/managing-ssh-keys
8. # 添加远程仓库
